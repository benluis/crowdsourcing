...
DATA_TO_SCORE_PATH = "validation.csv" # <-- CHANGE THIS
TEXT_COLUMN = "Text"
CALIBRATION_LABEL_COLUMN = "LABEL_A"
OUTPUT_FILE_PATH = "validation_scored_results.csv" # <-- CHANGE THIS
# -------------------
...
def calibrate_model(model, tokenizer, calibration_df):
    ...
    cal_texts = calibration_df[TEXT_COLUMN].tolist()
    cal_labels_list = calibration_df[CALIBRATION_LABEL_COLUMN].tolist()

    # --- THIS IS THE FIX: PROCESS IN BATCHES ---
    batch_size = 32
    all_logits = []

    print(f"Processing calibration data in batches of {batch_size}...")
    with torch.no_grad():
    ...
    cal_labels = torch.LongTensor(cal_labels_list).to(device)
    # --- END OF BATCHING FIX ---
...
def main():
    ...
    cal_df.dropna(subset=[TEXT_COLUMN, CALIBRATION_LABEL_COLUMN], inplace=True)
    ...
    df_to_score.dropna(subset=[TEXT_COLUMN], inplace=True)

    texts_to_score = df_to_score[TEXT_COLUMN].tolist()
